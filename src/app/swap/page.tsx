...
export default function SwapPage() {
  ...
  const getEstimate = React.useCallback(async () => {
    ...
    const [contractAddress, contractName] = router.id.split('.') as [string, string];
    ...
    const argsHex = [cvToHex(fromTokenCV), cvToHex(toTokenCV), cvToHex(fromAmountCV)];
  ...
  const handleSwap = async () => {
    ...
    const minToAmount = BigInt(toAmount) - (BigInt(toAmount) * BigInt(Math.floor(slippage * 100))) / BigInt(10000);
  ...
  return (
    <div className="min-h-screen w-full p-6 sm:p-10 space-y-8">
      ...
      <div className="rounded-lg border border-gray-200 dark:border-gray-700 p-4 space-y-4 max-w-md mx-auto">
        <div className="grid gap-3">
          <div>
            <div className="flex justify-between items-center text-xs mb-1">
              <label>From</label>
              <span>Balance: {fromTokenBalance ? formatAmount(fromTokenBalance.balance, fromTokenInfo?.decimals) : 0}</span>
            </div>
            <div className="flex gap-2">
              <select aria-label="From token" className="border rounded px-2 py-1 w-full" value={fromToken} onChange={handleFromTokenChange}>
                {Tokens.map(t => <option key={t.id} value={t.id}>{t.label}</option>)}
              </select>
              <input aria-label="From amount" className="border rounded px-2 py-1 w-full" value={formatAmount(fromAmount, fromTokenInfo?.decimals)} onChange={handleFromAmountChange} />
              <button onClick={() => setFromAmount(fromTokenBalance?.balance || "0")} className="text-xs px-2 py-1 rounded-md border border-gray-300 dark:border-gray-600">Max</button>
            </div>
          </div>
          <div className="text-center">
            <button onClick={invertTokens} className="p-1 rounded-full border border-gray-300 dark:border-gray-600">
          ...
          <div>
            ...
            <div className="flex gap-2">
              <select aria-label="To token" className="border rounded px-2 py-1 w-full" value={toToken} onChange={handleToTokenChange}>
                {Tokens.map(t => <option key={t.id} value={t.id}>{t.label}</option>)}
              </select>
              <input aria-label="To amount" className="border rounded px-2 py-1 w-full" value={formatAmount(toAmount, toTokenInfo?.decimals)} readOnly />
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between text-xs">
          ...
          <div className="flex items-center gap-2">
            <input type="number" value={slippage} onChange={e => setSlippage(parseFloat(e.target.value))} className="border rounded px-2 py-1 w-20 text-right" />
            <span>%</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-center">
          {isSignedIn ? (
            <button onClick={handleSwap} disabled={loading || sending || isSameToken} className="text-sm px-3 py-1.5 rounded-md border border-gray-300 dark:border-gray-600 flex items-center gap-2">
              {loading && <Spinner />}
              {sending ? "Sending..." : (loading ? "Getting estimate..." : (isSameToken ? "Select different tokens" : "Swap"))}
            </button>
          ) : (
            <button onClick={connectWallet} className="text-sm px-3 py-1.5 rounded-md border border-gray-300 dark:border-gray-600">
              Connect Wallet
            </button>
          )}
        </div>
        {status && <div role="status" aria-live="polite" className="text-xs text-gray-600 text-center">{status}</div>}
